...
consumption path. The worker source mirrors the same benchmark routes used by
the Prisma v7 and Drizzle v1 workers, and should be wired to the official
Prisma Next packages from the `vendor/prisma-next` submodule during the EA test.
...
3. Build and deploy this Worker with the same `DATABASE_URL` secret as the other
   two workers.
4. Re-run the same `/bench/*` load tests and compare Workers CPU/wall metrics.
